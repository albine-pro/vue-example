<template>
	<div class="auto_list__block">
        ...
        <div class="auto_list__block_w">
			...
			<div 
				class="auto_list__block_w_i" 
				v-for="model in models" 
				v-bind:key="model.id" 
				@click="changeActiveModelData(model.id)"
			>
...
export default {
  ...
  methods:{
    	changeActiveModelData(modelId){
     	...
     	moveAutoList(direction){
           ...
           if( direction && ( this.templateModelIndex + 2 ) === models.length ) return
  ...
  computed: { ...mapState(['models', 'miles', 'activeModel']) }
};
</script>
